...
async function launchForAccount(account, opts = {}) {
  ...
  if (opts.persistent) {
    ...
    // Harmless custom flag (Chrome/Chromium ignores unrecognized --wfm-* args)
    // that puts the account id INSIDE the OS process's own command line —
    // that's what lets lib/win-window.js find the right window reliably. Two
    // accounts on the same CRM vendor (e.g. perfectserve/uniters, both Five9)
    // render an identical generic page title ("Supervisor Desktop Plus"), so
    // a title-only search can't tell them apart and may act on the wrong one.
    // NOTE: do not use --user-data-dir here — Playwright's non-persistent
    // chromium.launch() manages its own temp profile and throws
    // "Pass userDataDir parameter to launchPersistentContext(...) instead"
    // if that flag is passed directly.
    browser = await chromium.launch({
      ...launchArgs,
      args: [...launchArgs.args, `--wfm-account-id=${account.id}`],
    });
