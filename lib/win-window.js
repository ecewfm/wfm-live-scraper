...
// ── Find Chrome PID by a unique marker embedded in its own command line ───────
// lib/browser.js passes --wfm-account-id=<account.id> (non-persistent
// launches) or a profile dir path containing the account id (persistent
// launches) on every launch, so matching the account id in CommandLine finds
// the exact window even when two accounts share an identical page title
// (e.g. perfectserve/uniters — both Five9, both "Supervisor Desktop Plus").
// This is checked first in hideWindow/showWindow, ahead of the title search.
function findPidByMarker(marker) {
  if (!marker) return null
  const safe = String(marker).replace(/'/g, '').replace(/"/g, '').substring(0, 80)
  try {
    const out = execSync(
      `powershell -NoProfile -NonInteractive -Command "` +
      `$c = Get-WmiObject Win32_Process -ErrorAction SilentlyContinue | ` +
      `Where-Object { $_.Name -like 'chrome*' -and $_.CommandLine -like '*${safe}*' -and $_.CommandLine -notlike '*--type=*' }; ` +
 ...
 * @param {string} [marker]  — account id embedded in the process's command line (see findPidByMarker)
 */
function hideWindow(browser, pageTitle, marker) {
  // 1. Try the command-line marker — the only strategy that can't be fooled by
  //    two accounts sharing an identical generic page title (e.g. two Five9 CRMs)
  if (marker) {
    const pid = findPidByMarker(marker)
    if (pid) return showWindowByPid(pid, 0)
  }
  // 2. Try page title search
  if (pageTitle) {
    const pid = findPidByTitle(pageTitle)
    if (pid) return showWindowByPid(pid, 0)
  }
  // 3. Try Playwright internal browser process APIs
  const pid2 = getPidFromBrowser(browser)
  if (pid2) return showWindowByPid(pid2, 0)
  // 4. Last resort — find any Chrome child of Node (may hit wrong window if multiple exist)
 ...
 * @param {string} [marker]  — account id embedded in the process's command line (see findPidByMarker)
 */
function showWindow(browser, pageTitle, marker) {
  if (marker) {
    const pid = findPidByMarker(marker)
