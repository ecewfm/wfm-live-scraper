// File: lib/terminal-dash.js
// Path: C:\Users\rodolfo.luga\Documents\Node Projects\wfm-live-scraper\lib\terminal-dash.js
...
class TerminalDash {
  constructor(accounts, intervalMs) {
    ...
    accounts.forEach(a => {
      this.states[a.id] = {
        ...
        info: 'not started — run: start ' + a.id, error: null
      }
    })
  }
  ...
  _drawBox() {
    const meta      = `${this.accounts.length} account${this.accounts.length !== 1 ? 's' : ''}  |  ${this.intervalMs / 1000}s interval  |  build ${BUILD}`
  ...
  // Add an account row dynamically and redraw the box
  addAccount(id) {
    if (this.accounts.some(a => a.id === id)) return
    this.accounts.push({ id })
    this.states[id] = { time: '--:--', ok: false, sla: '--', waiting: '0', agents: '--', info: 'not started — run: start ' + id, error: null }
    this._redraw()
  }

  // Remove an account row and redraw
  removeAccount(id) {
    this.accounts = this.accounts.filter(a => a.id !== id)
    delete this.states[id]
    delete this._acctRows[id]
    this._redraw()
  }

  // Redraw entire box after account list changes
  _redraw() {
    this._logRow = 9 + this.accounts.length
    this.accounts.forEach((a, i) => { this._acctRows[a.id] = 6 + i })
    const termH = process.stdout.rows || 40
