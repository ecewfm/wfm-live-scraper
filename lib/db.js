...
//
// The FIRST time a table is touched in this process's lifetime, the baseline
// is seeded from Supabase's EXISTING rows for that account instead of an
// empty set — otherwise a row that was already stale BEFORE this process
// even started (e.g. an agent who left days ago, under an older build that
// never pruned anything) would never enter tracking and would never be
// recognized as departed. No deletion happens on that seeding call itself,
// since an incomplete first live scrape (page still loading) could otherwise
// look like a mass departure — only ticks after the seed actually delete.
...
async function writeSnapshot(scraped, accountId) {
  ...
  if (scraped.agents && scraped.agents.length > 0) {
    ...
    await pruneDeparted('wfm_agent_states', accountId, new Set(agentRows.map(r => r.id)));
