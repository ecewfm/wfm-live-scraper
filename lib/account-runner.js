...
const { isFatalPageError } = require('./scrape-errors')

const SESSIONS_DIR = path.join(__dirname, '..', 'sessions')

// A per-tick backstop: if a module's scrape() ever hangs (never resolves or
// rejects — e.g. a Playwright call stuck on an undismissed dialog, or any
// other not-yet-seen wedge), the account's tick() would otherwise never reach
// its `finally` block, leaving `_ticking` stuck true forever — every future
// tick() becomes a silent no-op (`if (... this._ticking) return`), and even
// `force <id>` refuses to help ("already running here") since the runner
// still exists and looks like it's just busy. CONFIRMED as a real production
// incident (2026-09-18, the flex account, root-caused to an undismissed
// beforeunload dialog after a page.reload() — see scrapers/flex/index.js).
// Generous enough (3 minutes) to never false-trigger on a legitimately slow
// module (the slowest today, flex's Explore scrape, bounds itself well under
// this on its own) — this is purely a last-resort net for a genuine hang.
const SCRAPE_WATCHDOG_MS = 180000
function withTimeout(promise, ms, label) {
  return Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error(`${label} timed out after ${ms}ms — possible hang`)), ms)),
  ])
}
  ...
  async tick() {
    ...
    try {
      ...
      const data = await withTimeout(
        this.module.scrape(this.page, this.account), SCRAPE_WATCHDOG_MS, `[${this.account.id}] scrape()`
      )
      ...
      if (!data || data.hasData === false) {
        ...
        // supervisor page and revives the live widgets. Kept counting (never
        // reset to 0 by the retry below) so a configured emptyScrapeKillThreshold
        // (see config.json) can still fire after enough of these in-place
        // recoveries fail to actually revive the page.
        this._emptyStreak++
        ...
        // Opt-in per-account escalation (config.json's emptyScrapeKillThreshold)
        // for scrapers whose module can return an "empty" result even when the
        // underlying page/browser is actually dead (e.g. a scrape() that
        // swallows a closed-target error into null instead of throwing it —
        // see scrapers/perfectserve and scrapers/uniters) — _attemptRecovery()
        // only retries module.login() on the SAME page object, which can never
        // recover a truly gone page/context/browser. A full kill + relaunch
        // (same mechanism as the isFatalPageError branch below) is the only
        // thing that actually helps in that case.
        const killThreshold = this.account.emptyScrapeKillThreshold
        if (killThreshold && this._emptyStreak >= killThreshold) {
          this.dash.warn(this.account.id, `${this._emptyStreak} empty scrapes — killing and restarting browser (configured threshold: ${killThreshold})...`)
          this._emptyStreak = 0
          try { await this.browser?.close() } catch (_) {}
        ...
        if (this._emptyStreak > 0 && this._emptyStreak % 3 === 0) {
      ...
      if (isFatalPageError(err.message)) {
